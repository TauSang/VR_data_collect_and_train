...
class ACTChunkTransformerLite(nn.Module):
    """Domain-adaptive tokenized ACT-Chunk Transformer-Lite.

    The 31D observation is factorized into two semantic tokens:
      - proprio token: 8 joint positions + 8 joint velocities
      - task token: left/right EE position + three target-relative vectors

    A tiny TransformerEncoder models proprio-task interaction, then a FiLM MLP
    head keeps the proven domain-conditioning mechanism from ACTChunkMLPFiLM.
    The model intentionally uses only two tokens and a compact hidden head so it
    has transformer novelty without exceeding the current ~80K VR-frame regime.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if obs_dim <= proprio_dim:
            raise ValueError(f"obs_dim={obs_dim} must be larger than proprio_dim={proprio_dim}")
        self.obs_dim = obs_dim
        self.act_dim = act_dim
        self.chunk_size = chunk_size
        self.d_model = d_model
        self.proprio_dim = proprio_dim
        self.task_dim = obs_dim - proprio_dim
        self.num_domains = num_domains
        self.default_domain_id = default_domain_id
        ...
        self.task_proj = nn.Sequential(
            nn.Linear(self.task_dim, d_model),
            nn.LayerNorm(d_model),
            nn.GELU(),
            nn.Dropout(dropout),
        )
        self.type_emb = nn.Parameter(torch.zeros(1, 2, d_model))
        ...
        self.action_head = nn.Linear(prev, act_dim * chunk_size)
        self.success_head = nn.Sequential(
            nn.Linear(prev, max(prev // 4, 1)),
            nn.GELU(),
            nn.Linear(max(prev // 4, 1), 1),
        )

    def _domain_condition(self, obs: torch.Tensor, domain_id=None) -> torch.Tensor:
        B = obs.shape[0]
        if domain_id is None:
            dom = torch.full((B,), int(self.default_domain_id),
                             dtype=torch.long, device=obs.device)
    ...
    def forward(self, obs_seq, act_chunk=None, domain_id=None):
        ...
        task = obs[:, self.proprio_dim:]
        tokens = torch.stack([
            self.proprio_proj(proprio),
            self.task_proj(task),
        ], dim=1)
        tokens = tokens + self.type_emb
        tokens = self.encoder_norm(self.encoder(tokens))

        # Exactly two semantic tokens enter the encoder; mean-pool is stable and
        # avoids adding a third CLS token under the current data budget.
        h = tokens.mean(dim=1)
        cond = self._domain_condition(obs, domain_id)
        for blk in self.blocks:
            h = blk(h, cond)
...
class ACTChunkGatedCrossAttnFiLM(nn.Module):
    """Conservative v8 architecture: MLP-FiLM plus a gated cross-attention residual.

    v7 replaced the MLP observation path with a Transformer encoder and closed-loop
    SR collapsed. This class keeps the proven v2 MLP-FiLM path as the primary
    controller, then adds a proprio↔task attention residual at the input. The
    residual projection is zero-initialized so the network starts exactly as an
    ACTChunkMLPFiLM-style policy; the gate is initialized to a small non-zero
    value so gradients can flow into the attention projection from step 1. This
    is intentionally safer than v7 while keeping an attention-based architecture
    novelty for paper ablations.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attn_to_obs = nn.Linear(d_model, obs_dim)
        nn.init.zeros_(self.attn_to_obs.weight)
        nn.init.zeros_(self.attn_to_obs.bias)
    ...
    def shared_backbone_parameters(self):
        """Parameters to freeze during safe VR finetune.

        Leave FiLM adapters/domain embeddings and heads trainable; freeze the
        shared MLP/attention representation that anchors MuJoCo closed-loop
        behavior.
        """
        modules = [self.proprio_proj, self.task_proj, self.cross_attn,
                   self.attn_norm, self.attn_to_obs]
        for module in modules:
            yield from module.parameters()
        yield self.type_emb
        yield self.res_gate
        for blk in self.blocks:
            yield from blk.linear.parameters()
            yield from blk.norm.parameters()
    ...
    def forward(self, obs_seq, act_chunk=None, domain_id=None):
        ...
        attn_feat = self.attn_norm(tokens + attn_out).mean(dim=1)
        # Zero-initialized projection keeps the initial output exactly equivalent
        # to a plain MLP-FiLM path. The gate must stay small but non-zero;
        # otherwise zero gate + zero projection creates a gradient dead zone.
        obs_aug = obs + torch.tanh(self.res_gate) * self.attn_to_obs(attn_feat)
...
def main():
    ...
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=train_shuffle,
                              ...
                              persistent_workers=bool(cfg["train"].get("persistent_workers", False))
                                  if int(cfg["train"].get("num_workers", 0)) > 0 else False)
    ...
    backbone = str(act_cfg.get("backbone", "transformer")).lower()
    ...
    elif backbone in {"transformer_lite", "act_chunk_transformer_lite"}:
        model = ACTChunkTransformerLite(
            obs_dim=obs_dim,
            act_dim=act_dim,
            chunk_size=chunk_size,
            ...
            dropout=float(act_cfg.get("dropout", 0.2)),
            num_domains=int(act_cfg.get("num_domains", 2)),
            cond_dim=int(act_cfg.get("cond_dim", 16)),
            default_domain_id=int(act_cfg.get("default_domain_id", 0)),
            proprio_dim=int(act_cfg.get("proprio_dim", 16)),
        )
        model_class_name = "ACTChunkTransformerLite"
    elif backbone in {"gated_cross_attn_film", "cross_attn_film", "act_chunk_gated_cross_attn_film"}:
        model = ACTChunkGatedCrossAttnFiLM(
            obs_dim=obs_dim,
            act_dim=act_dim,
            chunk_size=chunk_size,
            ...
            nhead=int(act_cfg.get("nhead", 4)),
            attn_dropout=float(act_cfg.get("attn_dropout", 0.05)),
            residual_init=float(act_cfg.get("residual_init", 0.0)),
        )
        model_class_name = "ACTChunkGatedCrossAttnFiLM"
    else:
        model = ACTChunk(
            obs_dim=obs_dim,
            act_dim=act_dim,
    ...
    if pretrained_ckpt is not None:
        ...
        for key, param in pretrained_state.items():
            ...
            elif (key == "domain_emb.weight" and key in model_state
                  and model_state[key].dim() == 2 and param.dim() == 2
                  and model_state[key].shape[1] == param.shape[1]
                  and model_state[key].shape[0] >= param.shape[0]):
                # Allow extending a 2-domain checkpoint to 3+ domains without
                # resetting the already learned MJ/VR embeddings. Extra domains
                # start from the last pretrained domain embedding (VR-like) and
                # then adapt during finetune.
                expanded = model_state[key].clone()
                rows = param.shape[0]
                expanded[:rows] = param
                if expanded.shape[0] > rows:
                    expanded[rows:] = param[-1].unsqueeze(0).expand(expanded.shape[0] - rows, -1)
                model_state[key] = expanded
                loaded_keys.append(f"{key}[:{rows}]")
            else:
        ...
        if args.freeze_backbone:
            ...
            frozen = 0
            if hasattr(model, "shared_backbone_parameters"):
                for p in model.shared_backbone_parameters():
    ...
    # ── 20260425 v6: AMP (bf16 autocast, no GradScaler needed for bf16) ──
    use_amp = bool(cfg["train"].get("amp", False)) and torch.cuda.is_available()
    amp_dtype_name = str(cfg["train"].get("amp_dtype", "bfloat16")).lower()
    amp_dtype = {"bfloat16": torch.bfloat16, "bf16": torch.bfloat16,
                 "float16": torch.float16, "fp16": torch.float16}.get(amp_dtype_name, torch.bfloat16)
    print(f"[ACT-Chunk] AMP enabled={use_amp}, dtype={amp_dtype_name}")
            ...
            if use_amp:
                with torch.cuda.amp.autocast(dtype=amp_dtype):
                    ...
                    if loss_type == "huber":
                        ...
                        per_step_mse = hub.mean(dim=-1)
                        diff2 = (pred_actions - act_chunk) ** 2
                    else:
                        diff2 = (pred_actions - act_chunk) ** 2
                        per_step_mse = diff2.mean(dim=-1)
                    masked_mse = (per_step_mse * mask).sum(dim=-1) / mask.sum(dim=-1).clamp(min=1)
