...
class SegmentData:
    ...
    source_id: int = 0       # domain id (0=MuJoCo/primary, 1=VR, ...)
...
def load_segments(config: Dict) -> Tuple[List[SegmentData], Dict]:
    ...
    for src_idx, src in enumerate(sources):
        ...
        # Allow config override of source_id (domain index), else use list position
        domain_id = int(src.get("source_id", src_idx))
        for s in segs:
            s.source_id = domain_id
        summ["source_id"] = domain_id
