...
class TaskACTEncoder(nn.Module):
    """ACT with configurable readout: 'last_token' or 'multi_scale'."""
    ...
    def __init__(
        ...
        readout_mode: str = "multi_scale",
        readout_norm: bool = True,
    ):
        super().__init__()
        self.use_residual_action = use_residual_action
        ...
        if readout_mode == "multi_scale":
            self.readout_proj = nn.Sequential(
                nn.LayerNorm(d_model * 2) if readout_norm else nn.Identity(),
            ...
            # last_token readout
            self.readout_proj = nn.LayerNorm(d_model) if readout_norm else nn.Identity()
    ...
    def forward(self, obs_seq: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.readout_mode == "multi_scale":
            last_token = z[:, -1, :]
            mean_token = z.mean(dim=1)
            combined = torch.cat([last_token, mean_token], dim=-1)
            feat = self.readout_proj(combined)
        else:
            feat = self.readout_proj(z[:, -1, :])

        action_out = self.action_head(feat)
        success_out = self.success_head(feat).squeeze(-1)
        return action_out, success_out
...
def evaluate(model, loader, device, success_loss_weight: float):
    ...
    for obs_seq, act, success, weights in loader:
        obs_seq, act, success, weights = obs_seq.to(device), act.to(device), success.to(device), weights.to(device)
        pred_act, pred_success = model(obs_seq)
...
def main():
    ...
    x_train, y_train, s_train, w_train, _ = make_seq_dataset(
        train_segments, seq_len=seq_len, normalizer=normalizer,
        obs_clip_z=float(cfg["data"]["obs_clip_z"]),
        act_clip_z=float(cfg["data"]["act_clip_z"]),
    )
    x_val, y_val, s_val, w_val, _ = make_seq_dataset(
        val_segments, seq_len=seq_len, normalizer=normalizer,
        obs_clip_z=float(cfg["data"]["obs_clip_z"]),
        act_clip_z=float(cfg["data"]["act_clip_z"]),
    )
    ...
    act_cfg = cfg["act"]
    model = TaskACTEncoder(
        obs_dim=obs_dim, act_dim=act_dim,
        d_model=int(act_cfg["d_model"]), nhead=int(act_cfg["nhead"]),
        num_layers=int(act_cfg["num_layers"]), dim_feedforward=int(act_cfg["dim_feedforward"]),
        dropout=float(act_cfg["dropout"]),
        use_residual_action=bool(act_cfg.get("use_residual_action", False)),
        readout_mode=str(act_cfg.get("readout_mode", "multi_scale")),
        readout_norm=bool(act_cfg.get("readout_norm", True)),
    )
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[ACT] device={device}, d_model={act_cfg['d_model']}, layers={act_cfg['num_layers']}, heads={act_cfg['nhead']}")
    model.to(device)

    num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[ACT] trainable parameters: {num_params:,}")
    ...
    if obs_noise_std > 0:
        print(f"[ACT] obs noise augmentation: std={obs_noise_std}")

    for epoch in range(1, epochs + 1):
        model.train()
        train_total_losses, train_action_mses, train_success_bces, train_success_accs = [], [], [], []

        for obs_seq, act, success, weights in train_loader:
            obs_seq, act, success, weights = obs_seq.to(device), act.to(device), success.to(device), weights.to(device)
            # Observation noise augmentation during training
            if obs_noise_std > 0:
        ...
        if stale_epochs >= patience:
            print(f"[ACT] early stop at epoch={epoch} (patience={patience})")
            break
    ...
    plt.xlabel("epoch"); plt.ylabel("MSE"); plt.title("ACT Action Loss"); plt.grid(True, alpha=0.3); plt.legend()
    ...
    plt.xlabel("epoch"); plt.ylabel("Accuracy"); plt.title("ACT Success Aux"); plt.grid(True, alpha=0.3); plt.legend()

    plt.subplot(1, 3, 3)
    plt.plot(epochs_x, [h["lr"] for h in history])
    plt.xlabel("epoch"); plt.ylabel("LR"); plt.title("Learning Rate"); plt.grid(True, alpha=0.3)
    ...
    plt.xlabel("ground truth"); plt.ylabel("predicted"); plt.title("ACT Pred vs GT (Val, Best)")
    ...
    plt.xlabel("P(success)"); plt.ylabel("density"); plt.title("ACT Success Head (Val, Best)")
